...
class HDF5Dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        sar_pos_img = np.asarray(self.sar_pos_group[index]).astype(np.uint8)[:,:,:,0]
        optic_pos_img = np.asarray(self.optic_pos_group[index]).astype(np.uint8)[:,:,:,0]

        sar_sal_pos_img = np.asarray(self.sar_pos_group[index]).astype(np.uint8)[:,:,:,1]
        opt_sal_pos_img = np.asarray(self.optic_pos_group[index]).astype(np.uint8)[:,:,:,1]


        sar_neg_img = np.asarray(self.sar_neg_group[index]).astype(np.uint8)[:,:,:,0]
        optic_neg_img = np.asarray(self.optic_neg_group[index]).astype(np.uint8)[:,:,:,0]

        sar_sal_neg_img = np.asarray(self.sar_neg_group[index]).astype(np.uint8)[:,:,:,1]
        opt_sal_neg_img = np.asarray(self.optic_neg_group[index]).astype(np.uint8)[:,:,:,1]
        ...
        return (sar_pos_img, optic_pos_img, siamese_pos_label, sar_neg_img, optic_neg_img, siamese_neg_label, sar_sal_pos_img, opt_sal_pos_img, sar_sal_neg_img, opt_sal_neg_img)
...
class PseudoSiamese(nn.Module):
    ...
    def forward_sar(self, x, sar_sal_x):
    ...
    def forward_optic(self,x, opt_sal_x):
    ...
    def forward(self, sar_data, optic_data, sar_sal, optic_sal):
