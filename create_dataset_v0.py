...
def read_images_sequentially(path_sar, path_sar_sal, path_opt, path_opt_sal, sar_txt, opt_txt):
   ...
   #counter = 4
    ...
    for i in range(len(sar_txt)):
        ...
        s = cv2.imread(os.path.join(path_sar,converted_sar_name),0)
        s_s = cv2.imread(os.path.join(path_sar_sal,converted_sar_name),0)

        o = cv2.imread(os.path.join(path_opt,converted_opt_name),0)
        o_s = cv2.imread(os.path.join(path_opt_sal,converted_opt_name),0)
        ...
        if comparable_opt_name == comparable_sar_name:
            ...
            sarg_pos_train.append(sar_group)
            optg_pos_train.append(opt_group)

            labels_pos_train.append((0, 1))

        else:
            # means this sample is NOT positive sample == [1,0]
            sarg_neg_train.append(sar_group)
            optg_neg_train.append(opt_group)
    ...
    return [sarg_pos_train,optg_pos_train,sarg_neg_train,optg_neg_train,labels_pos_train,labels_neg_train]
...
if __name__=="__main__":
    ...
    # txt files
    sar_lines = tuple(open("D:/Sen1_2/Lists/Validation/list.val.supervised.sen12.sar.txt", 'r'))
    opt_lines = tuple(open("D:/Sen1_2/Lists/Validation/list.val.supervised.sen12.opt.txt", 'r'))
    ...
    sarg_pos_train,optg_pos_train,sarg_neg_train,optg_neg_train,l_pos_train,l_neg_train\
    = read_images_sequentially(sar_images_path,sar_sal_images_path,opt_images_path,opt_sal_images_path, sar_lines, opt_lines)

    # definition of h5 files
    hf_train = h5.File('D:/Sen1_2_/Validation_Matching.h5', 'w')


    # save them onto disk
    print("Lets save them onto hdf5  file")
    result = save_h5([hf_train], [sarg_pos_train,optg_pos_train,sarg_neg_train,optg_neg_train] ,
